...
export function show() {
  ...
  const title = isDefault ? t('notePrompt.demoTitle') : t('notePrompt.yourTitle');
  const meta = isDefault ? t('notePrompt.demoMeta') : t('notePrompt.uploadedMeta');
    ...
    <div class="chat-note-card">
      <div class="chat-note-card-header">
        ...
        <div class="chat-note-card-info">
          <div class="chat-note-card-title">${escapeHtml(title)}</div>
          <div class="chat-note-card-meta">${escapeHtml(meta)}</div>
        </div>
      </div>
      <div class="chat-note-card-body" id="note-card-preview-body">${escapeHtml(preview)}</div>
      ${hasMore ? `<button class="chat-note-card-toggle" id="note-card-toggle">${escapeHtml(t('notePrompt.showFull'))}</button>` : ''}
    </div>`;

  const chipDefs = isDefault
    ? [
        { value: 'proceed', label: t('notePrompt.looksGood') },
        { value: 'upload', label: t('notePrompt.uploadOwn') },
      ]
    : [
        { value: 'proceed', label: t('notePrompt.looksGood') },
        { value: 'upload', label: t('notePrompt.uploadDifferent') },
        { value: 'reset', label: t('notePrompt.resetDemo') },
      ];
  ...
  row.innerHTML = `
    ...
    <div class="msg-bubble">${escapeHtml(t('notePrompt.lead'))}${cardHtml}</div>`;
  ...
  chipRow.innerHTML = chipDefs.map((c, i) =>
    `<button type="button" class="chip" data-chip-value="${c.value}" style="animation-delay:${i * 0.08}s">${escapeHtml(c.label)}</button>`
  ).join('');
  chipRow.querySelectorAll('.chip').forEach(chip => {
    chip.addEventListener('click', () => selectNoteChip(chip.dataset.chipValue, chipRow));
  });
...
function toggleExpand() {
  ...
  btn.textContent = bodyEl.classList.contains('expanded') ? t('notePrompt.showLess') : t('notePrompt.showFull');
}
...
async function resetToDefault() {
  ...
  showToast(t('notePrompt.toastDemo'));
  if (onProceed) onProceed();
}
