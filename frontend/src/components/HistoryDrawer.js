...
function deriveSessionTitle(s) {
  ...
  if (!firstUser?.content) {
    if (s.mode === 'review') return t('history.titleReview');
    if (s.mode === 'prepare') return t('history.titlePrep');
    return t('history.titleChat');
  }
  const line = String(firstUser.content).replace(/\s+/g, ' ').trim();
  if (line.length <= 50) return line;
  const cut = line.lastIndexOf(' ', 50);
  return line.slice(0, cut > 20 ? cut : 50) + '\u2026';
}
  ...
  <div class="drawer-overlay" id="drawer-history">
    <div class="drawer">
      <div class="drawer-header">
        <div class="drawer-title">${escapeHtml(t('history.title'))}</div>
        <button class="icon-btn" id="drawer-close-btn" aria-label="${escapeHtml(t('version.close'))}">
...
export function renderItems() {
  ...
  if (!state.sessions.length) {
    body.innerHTML = `<div style="padding:24px 8px;text-align:center;color:var(--neutral-400);font-size:calc(14px * var(--type-scale));">${escapeHtml(t('history.empty'))}</div>`;
    return;
  }
  body.innerHTML = state.sessions.slice().reverse().map(s => `
    <div class="drawer-item" data-session-id="${s.id}">
      <div class="drawer-item-title">${escapeHtml(deriveSessionTitle(s))}</div>
      <div class="drawer-item-meta">${escapeHtml(t('history.meta', { date: s.date, count: s.messages.length }))}</div>
      <button class="session-delete-btn" title="${escapeHtml(t('history.delete'))}" data-delete-session="${s.id}">
